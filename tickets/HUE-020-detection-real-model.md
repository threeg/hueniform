...
- [ ] Real rembg + KMeans pipeline runs offline from the on-disk model (NFR-1, NFR-8)
- [ ] Real photo fixtures + sidecars committed incl. the garment-free fallback fixture
- [ ] `make test-model` passes on the owner's machine; skips with a clear message if the model is absent
- [ ] Tests added/updated per test strategy §12.2 (or exemption stated below) and passing in `make test`
- [ ] Relevant extra gate green where applicable (`make test-model` (§12.3.4))
