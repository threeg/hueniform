...
- 2026-06-16 — verified on owner's machine: `make test-model` → 5 passed, 647 deselected, 1 warning. The warning is the NFR-4 soft-assert for blank_bg (5.03 s > 5 s threshold, < 10 s hard cap) — correct behaviour. Note: the sanity command must use an absolute path for U2NET_HOME (rembg does not reliably resolve relative paths); use `make test-model` directly.
