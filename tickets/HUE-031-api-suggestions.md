...
- [ ] Found/zero/fail-fast response shapes match contract §2.12 exactly (FR-36–FR-43)
- [ ] `fallback: true` marks neutral-based fallbacks; zero result carries `explanation`+`hint` naming the constraint
- [ ] `409 empty_slots` and `422 invalid_request` returned per contract
- [ ] Tests added/updated per test strategy §12.2 (or exemption stated below) and passing in `make test`
- [ ] Relevant extra gate green where applicable (`make test-perf` for suggestion evaluation (§12.3.5))
