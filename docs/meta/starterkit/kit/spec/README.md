...
1. **Drop the kit into a new, empty repository.** It adds a `spec/` folder, a `.claude/skills/`
...
`.claude/skills/`, and `.gitignore`; the root `CLAUDE.md` and the `spec/` documents are generated by the
skills as they copy templates out and fill them. The project's own state — its project code and the kit
...
| 4 | **Wireframes** | `spec/wireframes/` — screens, states, navigation (skip for non-UI projects) | Authoring (Cowork) | `sfk-next-milestone` → `sfk-signoff` |
| 5 | **Test strategy** | `spec/test-strategy/test-strategy.md` — frameworks, the pyramid, gates, definition of done | Authoring (Cowork) | `sfk-next-milestone` → `sfk-signoff` |
| 6 | **Ticket generation** | `spec/tickets/*.md` + `CONVENTIONS.md` + `BOARD.md` — the work queue in dependency order | Authoring (Cowork) | `sfk-next-milestone` → `sfk-signoff` |
...
`spec/addons/meta-loop/` is **off by default**. Adopt it only if improving the development *method
