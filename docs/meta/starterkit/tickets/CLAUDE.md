...
- **Review after each batch.** Review the batch for reuse, quality and efficiency issues and propose
  cleanup tickets. Accepted tickets go to the cleanup backlog in `BOARD.md` (CONVENTIONS.md §6);
  critical ones are promoted into the main sequence before the gate they affect.
