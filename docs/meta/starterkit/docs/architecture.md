...
> and the *interface contract* disagree on shapes, the contract wins — see `03-api-contract.md`).
...
> `depends_on` graph must respect the same ordering (see `tickets/CONVENTIONS.md`).
