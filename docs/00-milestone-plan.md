...
| **Last updated** | 5 July 2026 (v0.2.0 shipped and tagged `v0.2.0`; v0.3.0 planning complete — Milestones 15–19 signed off (brief, design system with new NFR-11, architecture styling note, wireframe/responsive deltas with NFR-7 amended, test-strategy delta + tickets HUE-094–105); Milestone 20, implementation in Claude Code, is In progress) |
...
> **v0.3.0 planning complete; Milestones 15–19 signed off — Milestone 20 (implementation, in Claude Code) is In progress.** Build ticket by ticket with `sfk-next-ticket` in Claude Code (one ticket → one commit, per `tickets/CLAUDE.md`), following the `BOARD.md` v0.3.0 execution order from **HUE-094**. The milestone stays In progress until the tickets are all `done`, the gates pass and the user signs off (then v0.3.0 can be tagged). Milestone 16 delivered `docs/06-design-system.md` (role-named colour tokens, Newsreader/Hanken Grotesk/Space Mono typography on a normalised scale, spacing/radius/elevation, component vocabulary, native-CSS realisation with self-hosted fonts) and, from its requirement check, added binding **NFR-11** (accessibility: WCAG 2.1 AA contrast + visible focus). Milestone 17 confirmed the redesign is presentation-layer only — no change to modules, dependency rule, data model, flows or the HTTP contract — and added a frontend styling-architecture note to `docs/03-architecture.md` §2.5. Milestone 18 found the prototype includes **mobile and tablet layouts**, so **NFR-7 was amended** (desktop-only → responsive across mobile / tablet / desktop) — a presentation-only delta propagated to the requirements, design system and architecture — and the responsive layouts are specified in a new `docs/04-wireframes/07-responsive.md`; desktop screen structure is unchanged (a restyle). v0.3.0 now carries two NFR deltas: **NFR-11 (new)** and **NFR-7 (amended)**. Milestone 19 drafted the **test-strategy delta** (`docs/05-test-strategy.md` §10.3 — `jest-axe` for NFR-11, Playwright viewport journeys for NFR-7, self-hosted-fonts offline re-verification, **no screenshot diffing**) and **generated the v0.3.0 tickets**: epics **HUE-E11** (design-system foundation) and **HUE-E12** (screen restyle & responsive), leaf tickets **HUE-094–HUE-105**, with `tickets/BOARD.md` gaining a v0.3.0 execution-order section (dependency-checked: no forward references). Next, on sign-off: Milestone 20 — implementation, ticket by ticket, in Claude Code beginning with HUE-094. The brief is in `docs/10-v0.3.0-brief.md`: a **visual + layout redesign** applying the finished design in `docs/designs/Hueniform App.html` to the React SPA, captured as a binding design-system document (`docs/06-design-system.md`) with the prototype retained as the visual reference. It carries a **single requirement delta** — **NFR-11** (accessibility: WCAG 2.1 AA contrast + visible focus), added during the M16 design pass — with no `FR` or API-contract changes; all v0.2.0 behaviour and contracts are preserved (NFR-1/NFR-8 offline and NFR-6 responsiveness re-affirmed and re-verified). The design tokens are specified in `docs/06-design-system.md`. Proposed epics **HUE-E11** (design-system foundation) and **HUE-E12** (screen restyle & layout); implementation tickets continue from **HUE-094**. Per the scoping intent, each delta-pass milestone (M16–M20) is laid down and run through to confirm what it needs; M17 (architecture/API) is expected to be N/A. Next: Milestone 16 — design system + requirement check (`sfk-next-milestone`).
>
...
| 17 | Architecture & API deltas | Confirmed no API/contract change; added a frontend styling-architecture note to `docs/03-architecture.md` §2.5 (design-tokens layer, no CSS framework, self-hosted fonts, NFR-11) | **Cowork / Chat** | ✅ Complete |
| 18 | Wireframe deltas | Desktop structure unchanged (restyle); new `docs/04-wireframes/07-responsive.md` + overview updates; **NFR-7 amended** (responsive mobile/tablet/desktop) | **Cowork** | ✅ Complete |
| 19 | Test-strategy delta + ticket generation | `docs/05-test-strategy.md` §10.3 (jest-axe/NFR-11, Playwright viewports/NFR-7, no screenshot diffing); tickets HUE-094–HUE-105, epics E11–E12; `BOARD.md` v0.3.0 section | **Cowork** | ✅ Complete |
| 20 | Implementation, ticket by ticket | Working, redesigned SPA; tickets updated in the same commits (HUE-094–HUE-105) | **Code** | 🔶 In progress |
